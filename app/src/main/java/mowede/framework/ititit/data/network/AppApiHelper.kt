...
class AppApiHelper @Inject constructor(private val apiHeader: ApiHeader) : ApiHelper {
    override fun getListUserApiCall(): Observable<UserResponse> =
        Rx2AndroidNetworking.get(ApiEndPoint.ENDPOINT_GETUSERLIST)
        .addHeaders(apiHeader.publicApiHeader)
        .build()
        .getObjectObservable(UserResponse::class.java)

    override fun performServerRegister(request: LoginRequest.ServerRegisterRequest): Observable<LoginResponse> =
